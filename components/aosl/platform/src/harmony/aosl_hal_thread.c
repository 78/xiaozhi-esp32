...
{
	/* HarmonyOS NEXT (musl-based) does not support prctl(PR_SET_NAME).
	 * Use pthread_setname_np instead. */
...
int aosl_hal_thread_get_name(char *name, size_t size)
{
	...
	/* HarmonyOS NEXT (musl-based) does not support prctl(PR_GET_NAME).
	 * Use pthread_getname_np instead. */
