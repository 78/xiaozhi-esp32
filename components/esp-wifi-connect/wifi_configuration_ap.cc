...
void WifiConfigurationAp::StartSmartConfig()
{
    ...
    cfg.esp_touch_v2_key = const_cast<char *>("1234567890123456"); // 设置16字节加密密钥
                                                  ...
                                                  int32_t event_id, void *event_data)
{
    WifiConfigurationAp *self = static_cast<WifiConfigurationAp *>(arg);
    // 打印日志
    ESP_LOGI(TAG, "SmartConfig event: base=%s, id=%" PRId32, event_base, event_id);
    if (event_base == SC_EVENT)
    {
        switch (event_id)
        {
        case SC_EVENT_SCAN_DONE:
            ESP_LOGI(TAG, "SmartConfig scan done111111111111111111111111111111111111111111111111111");
            break;
        case SC_EVENT_FOUND_CHANNEL:
            ESP_LOGI(TAG, "Found SmartConfig channel111111111111111111111111111111111111111111111111111");
            break;
        case SC_EVENT_GOT_SSID_PSWD:
        {
            ESP_LOGI(TAG, "Got SmartConfig credentials111111111111111111111111111111111111111111111111111");

            smartconfig_event_got_ssid_pswd_t *evt = (smartconfig_event_got_ssid_pswd_t *)event_data;

            std::string ssid(reinterpret_cast<const char *>(evt->ssid));
            std::string password(reinterpret_cast<const char *>(evt->password));
            ESP_LOGI(TAG, "SmartConfig SSID: %s, Password: %s", ssid.c_str(), password.c_str());
        ...
        case SC_EVENT_SEND_ACK_DONE:
            ESP_LOGI(TAG, "SmartConfig ACK sent111111111111111111111111111111111111111111111111111");
            esp_smartconfig_stop();
            // 如果之前已收到SSID/Password，则保存并重启
            xTaskCreate([](void *ctx)
                        {
