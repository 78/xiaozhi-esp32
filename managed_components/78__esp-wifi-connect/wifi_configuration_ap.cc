...
char* WifiConfigurationAp::log_buffer_ = nullptr;
size_t WifiConfigurationAp::log_pos_ = 0;
std::mutex WifiConfigurationAp::log_mutex_;
vprintf_like_t WifiConfigurationAp::old_vprintf_handler_ = nullptr;

int WifiConfigurationAp::vprintf_handler(const char* format, va_list args) {
    char buf[256];
    ...
    if (len > 0) {
        ...
        if (log_buffer_ != nullptr) {
            for (int i = 0; i < len; i++) {
                log_buffer_[log_pos_] = buf[i];
                log_pos_ = (log_pos_ + 1) % LOG_BUFFER_SIZE;
            }
        }
    }
    return old_vprintf_handler_(format, args);
}
...
WifiConfigurationAp::WifiConfigurationAp()
{
    ...
    audio_pa_enable_ = -1;
    audio_volume_ = 70;
    display_orientation_ = 0;
    display_theme_ = "dark";
    display_brightness_ = 100;
}
...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    if (err == ESP_OK) {
        ...
        // 读取显示屏GPIO设置
        nvs_get_i8(nvs, "disp_mosi", &display_mosi_);
        nvs_get_i8(nvs, "disp_clk", &display_clk_);
        nvs_get_i8(nvs, "disp_dc", &display_dc_);
        nvs_get_i8(nvs, "disp_rst", &display_rst_);
        nvs_get_i8(nvs, "disp_cs", &display_cs_);
        nvs_get_i8(nvs, "disp_bl", &display_bl_);

        // 读取音频GPIO设置
        nvs_get_i8(nvs, "aud_bclk", &audio_i2s_bclk_);
        nvs_get_i8(nvs, "aud_ws", &audio_i2s_ws_);
        nvs_get_i8(nvs, "aud_dout", &audio_i2s_dout_);
        nvs_get_i8(nvs, "aud_din", &audio_i2s_din_);
        nvs_get_i8(nvs, "aud_pa", &audio_pa_enable_);
        nvs_get_i8(nvs, "aud_vol", &audio_volume_);

        // 读取显示外观设置
        nvs_get_i8(nvs, "disp_ori", &display_orientation_);
        nvs_get_i8(nvs, "disp_bri", &display_brightness_);
        
        nvs_handle_t display_nvs;
        if (nvs_open("display", NVS_READONLY, &display_nvs) == ESP_OK) {
            size_t theme_len = 0;
            if (nvs_get_str(display_nvs, "theme", nullptr, &theme_len) == ESP_OK) {
                char* theme_buf = (char*)malloc(theme_len);
                nvs_get_str(display_nvs, "theme", theme_buf, &theme_len);
                display_theme_ = theme_buf;
                free(theme_buf);
            }
            nvs_close(display_nvs);
        }
    ...
    if (log_buffer_ == nullptr) {
        log_buffer_ = (char*)malloc(LOG_BUFFER_SIZE);
        memset(log_buffer_, 0, LOG_BUFFER_SIZE);
        log_pos_ = 0;
        old_vprintf_handler_ = esp_log_set_vprintf(vprintf_handler);
    }
}
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t advanced_config = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // 添加显示屏GPIO
            cJSON_AddNumberToObject(json, "display_mosi", this_->display_mosi_);
            cJSON_AddNumberToObject(json, "display_clk", this_->display_clk_);
            cJSON_AddNumberToObject(json, "display_dc", this_->display_dc_);
            cJSON_AddNumberToObject(json, "display_rst", this_->display_rst_);
            cJSON_AddNumberToObject(json, "display_cs", this_->display_cs_);
            cJSON_AddNumberToObject(json, "display_bl", this_->display_bl_);

            // 添加音频GPIO
            cJSON_AddNumberToObject(json, "audio_i2s_bclk", this_->audio_i2s_bclk_);
            cJSON_AddNumberToObject(json, "audio_i2s_ws", this_->audio_i2s_ws_);
            cJSON_AddNumberToObject(json, "audio_i2s_dout", this_->audio_i2s_dout_);
            cJSON_AddNumberToObject(json, "audio_i2s_din", this_->audio_i2s_din_);
            cJSON_AddNumberToObject(json, "audio_pa_enable", this_->audio_pa_enable_);
            cJSON_AddNumberToObject(json, "audio_volume", this_->audio_volume_);

            // 添加显示外观
            cJSON_AddNumberToObject(json, "display_orientation", this_->display_orientation_);
            cJSON_AddStringToObject(json, "display_theme", this_->display_theme_.c_str());
            cJSON_AddNumberToObject(json, "display_brightness", this_->display_brightness_);
    ...
    httpd_uri_t advanced_submit_uri = {
        .uri = "/advanced/submit",
        .method = HTTP_POST,
        .handler = [](httpd_req_t *req) -> esp_err_t {
            char *buf;
            ...
            // 保存显示屏GPIO设置
            auto save_gpio = [&](const char* json_key, const char* nvs_key, int8_t& member) {
                cJSON *item = cJSON_GetObjectItem(json, json_key);
                if (cJSON_IsNumber(item)) {
                    member = (int8_t)item->valueint;
                    nvs_set_i8(nvs, nvs_key, member);
                }
            };
            save_gpio("display_mosi", "disp_mosi", this_->display_mosi_);
            save_gpio("display_clk", "disp_clk", this_->display_clk_);
            save_gpio("display_dc", "disp_dc", this_->display_dc_);
            save_gpio("display_rst", "disp_rst", this_->display_rst_);
            save_gpio("display_cs", "disp_cs", this_->display_cs_);
            save_gpio("display_bl", "disp_bl", this_->display_bl_);

            // 保存音频GPIO设置
            save_gpio("audio_i2s_bclk", "aud_bclk", this_->audio_i2s_bclk_);
            save_gpio("audio_i2s_ws", "aud_ws", this_->audio_i2s_ws_);
            save_gpio("audio_i2s_dout", "aud_dout", this_->audio_i2s_dout_);
            save_gpio("audio_i2s_din", "aud_din", this_->audio_i2s_din_);
            save_gpio("audio_pa_enable", "aud_pa", this_->audio_pa_enable_);
            save_gpio("audio_volume", "aud_vol", this_->audio_volume_);

            // 保存显示外观设置
            save_gpio("display_orientation", "disp_ori", this_->display_orientation_);
            save_gpio("display_brightness", "disp_bri", this_->display_brightness_);
            
            cJSON* theme_item = cJSON_GetObjectItem(json, "display_theme");
            if (cJSON_IsString(theme_item)) {
                this_->display_theme_ = theme_item->valuestring;
                nvs_handle_t display_nvs;
                if (nvs_open("display", NVS_READWRITE, &display_nvs) == ESP_OK) {
                    nvs_set_str(display_nvs, "theme", this_->display_theme_.c_str());
                    nvs_commit(display_nvs);
                    nvs_close(display_nvs);
                }
            }
    ...
    httpd_uri_t reboot_uri = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            ESP_LOGI("WifiConfigurationAp", "Rebooting in 1 second...");
    ...
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &reboot_uri));

    // Register the /reset_wifi URI
    ...
    httpd_uri_t log_data_uri = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // Send in two parts if wrapped
            size_t pos = log_pos_;
            httpd_resp_send_chunk(req, log_buffer_ + pos, LOG_BUFFER_SIZE - pos);
            httpd_resp_send_chunk(req, log_buffer_, pos);
