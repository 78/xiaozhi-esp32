 ...
 */
 ...
 static esp_err_t ov02c10_set_format(esp_cam_sensor_device_t *dev, const esp_cam_sensor_format_t *format)
 {
     ...
     // reset
     ret = ov02c10_write_array(dev->sccb_handle, ov02c10_mipi_reset_regs);
     ...
     // ret = ov02c10_set_AE_target(dev, OV02C10_AE_TARGET_DEFAULT);
     // ESP_RETURN_ON_FALSE(ret == ESP_OK, ret, TAG, "set ae target failed");
     ov02c10_set_bandingfilter(dev);
 
     // stop stream default
     ret = ov02c10_set_stream(dev, 0);
     ESP_RETURN_ON_FALSE(ret == ESP_OK, ret, TAG, "write stream regs failed");
     ESP_LOGD(TAG, "light freq=0x%x", ov02c10_get_light_freq(dev));
