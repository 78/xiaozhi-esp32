...
#define TAG "XINGZHI_CUBE_0_96OLED_WIFI"  // 定义日志标签

LV_FONT_DECLARE(font_puhui_14_1);  // 声明字体
LV_FONT_DECLARE(font_awesome_14_1);  // 声明图标字体

// CustomDisplay 类继承自 Ssd1306Display，用于自定义显示功能
...
// XINGZHI_CUBE_0_96OLED_WIFI 类继承自 WifiBoard，用于实现具体的硬件功能
class XINGZHI_CUBE_0_96OLED_WIFI : public WifiBoard {
private:
    i2c_master_bus_handle_t display_i2c_bus_;  // I2C 总线句柄
    ...
    void InitializePowerSaveTimer() {
        rtc_gpio_init(GPIO_NUM_21);  // 初始化 RTC GPIO
        rtc_gpio_set_direction(GPIO_NUM_21, RTC_GPIO_MODE_OUTPUT_ONLY);  // 设置 GPIO 方向为输出
        rtc_gpio_set_level(GPIO_NUM_21, 1);  // 设置 GPIO 电平为高
        ...
        power_save_timer_->OnShutdownRequest([this]() {  // 关机请求时的回调
    ...
    void InitializeDisplayI2c() {
        i2c_master_bus_config_t bus_config = {  // I2C 总线配置
        ...
        ESP_ERROR_CHECK(i2c_new_master_bus(&bus_config, &display_i2c_bus_));  // 创建 I2C 主总线
    }
    ...
    void InitializeButtons() {
        ...
        volume_up_button_.OnClick([this]() {  // 音量增加按钮点击事件
            power_save_timer_->WakeUp();  // 唤醒设备
        ...
        volume_up_button_.OnLongPress([this]() {  // 音量增加按钮长按事件
            power_save_timer_->WakeUp();  // 唤醒设备
            GetAudioCodec()->SetOutputVolume(100);  // 设置音量为最大
            GetDisplay()->ShowNotification(Lang::Strings::MAX_VOLUME);  // 显示最大音量通知
        });

        volume_down_button_.OnClick([this]() {  // 音量减少按钮点击事件
            power_save_timer_->WakeUp();  // 唤醒设备
            auto codec = GetAudioCodec();
        ...
        volume_down_button_.OnLongPress([this]() {  // 音量减少按钮长按事件
            power_save_timer_->WakeUp();  // 唤醒设备
            GetAudioCodec()->SetOutputVolume(0);  // 设置音量为静音
...
public:
    ...
    XINGZHI_CUBE_0_96OLED_WIFI() :
        ...
        InitializeDisplayI2c();  // 初始化显示 I2C
        InitializeButtons();  // 初始化按钮
        InitializeIot();  // 初始化 IoT 设备
    }
    ...
    virtual void SetPowerSaveMode(bool enabled) override {
        ...
        WifiBoard::SetPowerSaveMode(enabled);  // 设置节能模式
    }
};
