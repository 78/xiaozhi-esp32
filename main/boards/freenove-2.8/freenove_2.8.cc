...
private:
    ...
    void InitializeAdc() {
        ...
        ESP_ERROR_CHECK(adc_oneshot_config_channel(adc_handle_, ADC_CHANNEL_8, &config)); // GPIO 9 is ADC1_CH8
    }
...
public:
    ...
    virtual bool GetBatteryLevel(int &level, bool& charging, bool& discharging) override {
        int r;
        if (adc_oneshot_read(adc_handle_, ADC_CHANNEL_8, &r) == ESP_OK) {
            float voltage = r * 0.00175f; // Calibration for Freenove S3 (standard 2:1 divider)
            ESP_LOGI(TAG, "Battery ADC: %d, Voltage: %.2fV", r, voltage);
            
            // USB Detection: If voltage > 4.20V, it's typically powered by USB/Charging
            if (voltage > 4.20f || (usb_detected_ && voltage < 0.1f)) {
                usb_detected_ = true;
                charging = true;
                discharging = false;
                level = 100;
                ...
                else level = (int)((voltage - 3.3f) * 111.1f);
            }
            return true;
        }
        return false;
