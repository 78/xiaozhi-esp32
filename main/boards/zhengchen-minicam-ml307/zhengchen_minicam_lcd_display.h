...
public:
    ...
    void SetReprovisionHandlers(std::function<bool()> visible_getter, std::function<void()> callback);
    void SetSwitchNetworkCallback(std::function<void()> callback);
...
private:
    ...
    std::function<bool()> reprovision_visible_getter_;
    std::function<void()> reprovision_callback_;
    std::function<void()> switch_network_callback_;
