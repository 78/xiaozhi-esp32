...
void ZhengchenMinicamLcdDisplay::SelectPreviousMenuItem() {
    ...
    // 跳过隐藏项（4G 模式下 Reprovision 不可见）
    if (selected_index_ == static_cast<int>(MenuItem::Reprovision) &&
        reprovision_visible_getter_ != nullptr && !reprovision_visible_getter_()) {
        SetSelectedIndex(selected_index_ - 1);
    }
    RefreshSettingsMenuLocked();
}
...
void ZhengchenMinicamLcdDisplay::ActivateSelectedMenuItem() {
    ...
    case MenuItem::Reprovision:
        if (reprovision_visible_getter_ != nullptr && !reprovision_visible_getter_()) {
            break;
        }
        ...
        break;
    case MenuItem::SwitchNetwork:
        if (switch_network_callback_) {
            HideSettingsMenu();
            switch_network_callback_();
            return;
        }
        break;
    case MenuItem::Count:
...
void ZhengchenMinicamLcdDisplay::SetReprovisionHandlers(std::function<bool()> visible_getter, std::function<void()> callback) {
    reprovision_visible_getter_ = std::move(visible_getter);
    reprovision_callback_ = std::move(callback);
}

void ZhengchenMinicamLcdDisplay::SetSwitchNetworkCallback(std::function<void()> callback) {
    switch_network_callback_ = std::move(callback);
}
...
void ZhengchenMinicamLcdDisplay::RefreshSettingsMenuLocked() {
    ...
    auto text_color = lvgl_theme->text_color();
    auto bg_color = lvgl_theme->background_color();
    const bool reprovision_visible = reprovision_visible_getter_ == nullptr || reprovision_visible_getter_();
    ...
    for (size_t i = 0; i < menu_item_labels_.size(); ++i) {
        ...
        if (item == MenuItem::Reprovision && !reprovision_visible) {
        ...
        lv_label_set_text(label, GetMenuLabel(item));
