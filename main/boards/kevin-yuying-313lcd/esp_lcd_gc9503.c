...
// 定义GC9503 LCD面板的命令和默认值
#define GC9503_CMD_MADCTL (0xB1)         // 内存数据访问控制命令
#define GC9503_CMD_MADCTL_DEFAULT (0x10) // 内存数据访问控制的默认值
#define GC9503_CMD_SS_BIT (1 << 0)       // 源驱动器扫描方向，0：从上到下，1：从下到上
#define GC9503_CMD_GS_BIT (1 << 1)       // 栅极驱动器扫描方向，0：从左到右，1：从右到左
#define GC9503_CMD_BGR_BIT (1 << 5)      // RGB/BGR顺序，0：RGB，1：BGR

// 定义GC9503面板的结构体，用于存储面板的状态和配置
typedef struct
{
    esp_lcd_panel_io_handle_t io; // LCD面板的IO句柄
    int reset_gpio_num;           // 复位GPIO引脚号
    uint8_t madctl_val;           // 当前GC9503_CMD_MADCTL寄存器的值
    uint8_t colmod_val;           // 当前LCD_CMD_COLMOD寄存器的值
    const gc9503_lcd_init_cmd_t *init_cmds; // 初始化命令数组
    uint16_t init_cmds_size;                // 初始化命令数组的大小
    struct
    {
        unsigned int mirror_by_cmd : 1;           // 是否通过命令控制镜像
        unsigned int auto_del_panel_io : 1;       // 是否自动删除面板IO
        unsigned int display_on_off_use_cmd : 1;  // 是否通过命令控制显示开关
...
static const char *TAG = "gc9503"; // 日志标签
