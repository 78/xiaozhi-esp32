...
private:
    ...
    void InitializeTools() {
        ...
        mcp_server.AddTool(
            ...
            "Information about the battery, including state of charge, "
            "voltage, current, temperature, capacity, state of health, and "
            "charge status.",
            PropertyList(),
            [this](const PropertyList &properties) -> ReturnValue {
                ...
                ESP_LOGD(TAG,
                         "Battery info: SOC=%d%%, voltage=%dmV, current=%dmA, "
                         "temperature=%d℃, capacity=%dmAh, charging=%d",
                         soc, voltage, current, temperature, capacity,
                         is_charging);
                cJSON *json = cJSON_CreateObject();
    ...
    void InitializeBatteryMonitor() {
        ...
        battery_monitor.setBatteryStatusCallback(
            [this](const battery_status_t &status) {
