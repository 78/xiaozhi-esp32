...
static int current_icon_type = MMAP_EMOJI_ICON_BATTERY_BIN;
...
static void clock_tm_callback(void* user_data)
{
    ...
    if (current_icon_type == MMAP_EMOJI_ICON_BATTERY_BIN) {
        time_t now;
        struct tm timeinfo;
        time(&now);
...
static void InitializeEyeAnimation(gfx_handle_t engine_handle, mmap_assets_handle_t assets_handle)
{
    ...
    const void* anim_data = mmap_assets_get_mem(assets_handle, MMAP_EMOJI_IDLE_ONE_AAF);
    size_t anim_size = mmap_assets_get_size(assets_handle, MMAP_EMOJI_IDLE_ONE_AAF);
...
static void InitializeFont(gfx_handle_t engine_handle, mmap_assets_handle_t assets_handle)
{
    ...
    gfx_label_cfg_t font_cfg = {
        .name = "DejaVuSans.ttf",
        .mem = mmap_assets_get_mem(assets_handle, MMAP_EMOJI_KAITI_TTF),
        .mem_size = static_cast<size_t>(mmap_assets_get_size(assets_handle, MMAP_EMOJI_KAITI_TTF)),
    };
...
static void InitializeMicAnimation(gfx_handle_t engine_handle, mmap_assets_handle_t assets_handle)
{
    ...
    const void* anim_data = mmap_assets_get_mem(assets_handle, MMAP_EMOJI_LISTEN_AAF);
    size_t anim_size = mmap_assets_get_size(assets_handle, MMAP_EMOJI_LISTEN_AAF);
...
static void InitializeIcon(gfx_handle_t engine_handle, mmap_assets_handle_t assets_handle)
{
    ...
    SetupImageDescriptor(assets_handle, &icon_img_dsc, MMAP_EMOJI_ICON_WIFI_FAILED_BIN);
...
EmoteEngine::EmoteEngine(esp_lcd_panel_handle_t panel, esp_lcd_panel_io_handle_t panel_io)
{
    ...
    current_icon_type = MMAP_EMOJI_ICON_WIFI_FAILED_BIN;
...
void EmoteDisplay::SetEmotion(const char* emotion)
{
    ...
    static const std::unordered_map<std::string, EmotionParam> emotion_map = {
        {"happy",       {MMAP_EMOJI_HAPPY_ONE_AAF,     true,  20}},
        {"laughing",    {MMAP_EMOJI_ENJOY_ONE_AAF,     true,  20}},
        {"funny",       {MMAP_EMOJI_HAPPY_ONE_AAF,     true,  20}},
        {"loving",      {MMAP_EMOJI_HAPPY_ONE_AAF,     true,  20}},
        {"embarrassed", {MMAP_EMOJI_HAPPY_ONE_AAF,     true,  20}},
        {"confident",   {MMAP_EMOJI_HAPPY_ONE_AAF,     true,  20}},
        {"delicious",   {MMAP_EMOJI_HAPPY_ONE_AAF,     true,  20}},
        {"sad",         {MMAP_EMOJI_SAD_ONE_AAF,       true,  20}},
        {"crying",      {MMAP_EMOJI_HAPPY_ONE_AAF,     true,  20}},
        {"sleepy",      {MMAP_EMOJI_HAPPY_ONE_AAF,     true,  20}},
        {"silly",       {MMAP_EMOJI_HAPPY_ONE_AAF,     true,  20}},
        {"angry",       {MMAP_EMOJI_ANGRY_ONE_AAF,     true,  20}},
        {"surprised",   {MMAP_EMOJI_HAPPY_ONE_AAF,     true,  20}},
        {"shocked",     {MMAP_EMOJI_SHOCKED_ONE_AAF,   true,  20}},
        {"thinking",    {MMAP_EMOJI_THINKING_ONE_AAF,  true,  20}},
        {"winking",     {MMAP_EMOJI_HAPPY_ONE_AAF,     true,  20}},
        {"relaxed",     {MMAP_EMOJI_HAPPY_ONE_AAF,     true,  20}},
        {"confused",    {MMAP_EMOJI_DIZZY_ONE_AAF,     true,  20}},
        {"neutral",     {MMAP_EMOJI_IDLE_ONE_AAF,      false, 20}},
        {"idle",        {MMAP_EMOJI_IDLE_ONE_AAF,      false, 20}},
    };
...
void EmoteDisplay::SetStatus(const char* status)
{
    ...
    if (std::strcmp(status, "聆听中...") == 0) {
        ...
        engine_->setEyes(MMAP_EMOJI_HAPPY_ONE_AAF, true, 20);
        engine_->SetIcon(MMAP_EMOJI_ICON_MIC_BIN);
    } else if (std::strcmp(status, "待命") == 0) {
        SetUIDisplayMode(UIDisplayMode::SHOW_TIME);
        engine_->SetIcon(MMAP_EMOJI_ICON_BATTERY_BIN);
    } else if (std::strcmp(status, "说话中...") == 0) {
        SetUIDisplayMode(UIDisplayMode::SHOW_TIPS);
        engine_->SetIcon(MMAP_EMOJI_ICON_SPEAKER_ZZZ_BIN);
    } else if (std::strcmp(status, "错误") == 0) {
        SetUIDisplayMode(UIDisplayMode::SHOW_TIPS);
        engine_->SetIcon(MMAP_EMOJI_ICON_WIFI_FAILED_BIN);
    }
