...
void BatteryMonitor::printInfo() const {
    ...
    ESP_LOGI(TAG,
             "Battery Status - DSG: %d, SYSDWN: %d, TDA: %d, BATTPRES: %d, "
             "AUTH_GD: %d, OCVGD: %d, TCA: %d, RSVD: %d, CHGINH: %d, FC: %d, "
             "OTD: %d, OTC: %d, SLEEP: %d, OCVFAIL: %d, OCVCOMP: %d, FD: %d",
             status.DSG, status.SYSDWN, status.TDA, status.BATTPRES,
             status.AUTH_GD, status.OCVGD, status.TCA, status.RSVD,
             status.CHGINH, status.FC, status.OTD, status.OTC, status.SLEEP,
             status.OCVFAIL, status.OCVCOMP, status.FD);
    ...
    ESP_LOGI(TAG,
             "Battery Info - Vol: %dmv, Current: %dmA, Power: %dmW, Remaining "
             "Capacity: %dmAh, Full Charge Capacity: %dmAh, Temperature: %dC, "
             "Cycle Count: %d, SOC: %d%%, Max Load: %dmA, Time to empty: "
             "%dmin, Time to full: %dmin",
             vol, current, avg_power, rc, full_cap, temp, cycle_cnt, soc,
             max_load, time_to_empty, time_to_full);
}
