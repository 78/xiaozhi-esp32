...
#define TAG "XIAOZHI_TFT_AICHATBOX_WIFI"
#define LCD_LEDC_CH LEDC_CHANNEL_0
// #define PIN_NUMBER GPIO_NUM_21   //LDO-EN控制引脚

LV_FONT_DECLARE(font_awesome_30_4);

XIAOZHI_1_54_TFT_LcdDisplay::XIAOZHI_1_54_TFT_LcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
      ...
      fonts_(fonts), last_interaction_time_(esp_timer_get_time()), boot_button_(BOOT_BUTTON_GPIO), volume_up_button_(VOLUME_UP_BUTTON_GPIO), volume_down_button_(VOLUME_DOWN_BUTTON_GPIO)
{
    ...
    const esp_timer_create_args_t timer_args = {
        .callback = [](void *arg)
        {
            XIAOZHI_1_54_TFT_LcdDisplay *display = static_cast<XIAOZHI_1_54_TFT_LcdDisplay *>(arg);
    ...
    esp_timer_create_args_t charging_timer_args = {
        .callback = &XIAOZHI_1_54_TFT_LcdDisplay::ChargingTimerCallback,
        .arg = this,
        .dispatch_method = ESP_TIMER_TASK,
        .name = "charging_timer"};
    ...
    esp_timer_create_args_t battery_timer_args = {
        .callback = &XIAOZHI_1_54_TFT_LcdDisplay::BatteryTimerCallback,
        .arg = this,
        .dispatch_method = ESP_TIMER_TASK,
        .name = "battery_timer"};
...
XIAOZHI_1_54_TFT_LcdDisplay::~XIAOZHI_1_54_TFT_LcdDisplay()
{
...
void XIAOZHI_1_54_TFT_LcdDisplay::SetBacklight(uint8_t brightness)
{
...
void XIAOZHI_1_54_TFT_LcdDisplay::InitializeBacklight(gpio_num_t backlight_pin)
{
...
void XIAOZHI_1_54_TFT_LcdDisplay::OnBacklightTimer()
{
...
void XIAOZHI_1_54_TFT_LcdDisplay::UpdateInteractionTime()
{
...
// 睡眠状态检测
void XIAOZHI_1_54_TFT_LcdDisplay::CheckSleepState()
{
    ...
    // 第30秒后未开始充电，降低屏幕亮度
    if (elapsed_time >= 32 && !is_light_run_ && !is_deep_run_)
    {
        is_light_run_ = true;
        SetBacklight(1);
    }
    // 第60秒后未开始充电，直接关闭LDO停止电压输出，控制板个位数2-3uA的静态电流
    else if (elapsed_time >= 62 && is_light_run_)
    {
        is_deep_run_ = true;
        is_light_run_ = false;

        // LDO-EN控制引脚
        gpio_config_t ldoen_io_conf = {};
        ldoen_io_conf.mode = GPIO_MODE_OUTPUT;
        ldoen_io_conf.pin_bit_mask = (1ULL << ldoen_pin_);
        gpio_config(&ldoen_io_conf);
        gpio_set_level(ldoen_pin_, 0); // 拉低LDO-EN引脚，关闭电源
        vTaskDelay(3000 / portTICK_PERIOD_MS);
    }
}

bool XIAOZHI_1_54_TFT_LcdDisplay::Lock(int timeout_ms)
{
    return lvgl_port_lock(timeout_ms);
}

void XIAOZHI_1_54_TFT_LcdDisplay::Unlock()
{
    lvgl_port_unlock();
}

void XIAOZHI_1_54_TFT_LcdDisplay::SetupUI()
{
...
void XIAOZHI_1_54_TFT_LcdDisplay::UpdateBatteryAndChargingDisplay(uint16_t average_adc)
{
    DisplayLockGuard lock(this);
    // 未充电时，显示电池图标
    ...
    else if (average_adc >= 1800 && average_adc < 2100)
    {
        battery_level = 1;
        // 如果电量回升，隐藏提示窗口
    ...
    else if (average_adc >= 2100 && average_adc < 2300)
    {
        battery_level = 2;
        if (low_battery_popup_ != nullptr)
        {
    ...
    else if (average_adc >= 2300 && average_adc < 2400)
    {
        battery_level = 3;
        if (low_battery_popup_ != nullptr)
        {
...
// lcd_display.c
void XIAOZHI_1_54_TFT_LcdDisplay::ShowLowBatteryPopup()
{
    DisplayLockGuard lock(this);
    if (low_battery_popup_ == nullptr)
    {
        low_battery_popup_ = lv_obj_create(lv_scr_act());
        lv_obj_set_size(low_battery_popup_, LV_HOR_RES * 0.95, LV_VER_RES * 0.5);
        lv_obj_center(low_battery_popup_);
        lv_obj_set_style_bg_color(low_battery_popup_, LV_COLOR_MAKE(255, 128, 0), 0);
        lv_obj_set_style_radius(low_battery_popup_, 10, 0);

        low_battery_label_ = lv_label_create(low_battery_popup_);
        lv_label_set_text_fmt(low_battery_label_, "电量偏低 请立即充电\n避免损坏电池\n%d秒后自动关机", turn_ldo_off_times);
        lv_obj_set_style_text_color(low_battery_label_, lv_color_white(), 0);
        lv_obj_set_style_text_align(low_battery_label_, LV_TEXT_ALIGN_CENTER, 0);
        lv_obj_center(low_battery_label_);

        // 创建一个定时器，每秒减少 turn_ldo_off_times 的值
        const esp_timer_create_args_t timer_args = {
            .callback = [](void *arg)
            {
                XIAOZHI_1_54_TFT_LcdDisplay *display = static_cast<XIAOZHI_1_54_TFT_LcdDisplay *>(arg);
                display->UpdateLowBatteryPopup();
            },
            .arg = this,
            .dispatch_method = ESP_TIMER_TASK,
        ...
        ESP_ERROR_CHECK(esp_timer_create(&timer_args, &low_battery_timer_));
        ESP_ERROR_CHECK(esp_timer_start_periodic(low_battery_timer_, 1000000)); // 每秒触发一次
    }
...
void XIAOZHI_1_54_TFT_LcdDisplay::UpdateLowBatteryPopup()
{
    DisplayLockGuard lock(this);
    if (turn_ldo_off_times > 0)
    {
        turn_ldo_off_times--;
        lv_label_set_text_fmt(low_battery_label_, "电量偏低 请立即充电\n避免损坏电池\n%d秒后自动关机", turn_ldo_off_times);
    }
...
uint16_t XIAOZHI_1_54_TFT_LcdDisplay::ReadBatteryLevel()
{   
    // 初始化 ADC 单元
...
void XIAOZHI_1_54_TFT_LcdDisplay::ChargingTimerCallback(void *arg)
{
    XIAOZHI_1_54_TFT_LcdDisplay *display = static_cast<XIAOZHI_1_54_TFT_LcdDisplay *>(arg);
...
void XIAOZHI_1_54_TFT_LcdDisplay::BatteryTimerCallback(void *arg)
{
    XIAOZHI_1_54_TFT_LcdDisplay *display = static_cast<XIAOZHI_1_54_TFT_LcdDisplay *>(arg);
    uint16_t adc_value = display->ReadBatteryLevel();
    if (display->first_battery_invert_)
    {
        display->adc_samp_interval = 500000; // adc值采样的时间间隔
        // 停止当前定时器
...
void XIAOZHI_1_54_TFT_LcdDisplay::StartChargingTimer()
{
    ESP_ERROR_CHECK(esp_timer_start_periodic(charging_timer_, adc_samp_interval));
}

void XIAOZHI_1_54_TFT_LcdDisplay::StartBatteryTimer()
{
    ESP_ERROR_CHECK(esp_timer_start_periodic(battery_timer_, adc_samp_interval));
}

void XIAOZHI_1_54_TFT_LcdDisplay::SetEmotion(const char *emotion)
{
    struct Emotion
    {
        const char *icon;
...
void XIAOZHI_1_54_TFT_LcdDisplay::SetIcon(const char *icon)
{
    DisplayLockGuard lock(this);
    if (emotion_label_ == nullptr)
    {
...
void XIAOZHI_1_54_TFT_LcdDisplay::OnStateChanged()
{
