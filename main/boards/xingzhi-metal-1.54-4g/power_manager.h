...
private:
    ...
    void CheckBatteryStatus() {
        // 唤醒4g失败，每隔5s重试一次
        if (!modem_awake && !no_modem) {
            number_++;
            if (number_ >= 5) {
                bool wake_success = Exit_SleepModem();
                if (wake_success) {
                    modem_awake = true; 
                } else {
                    number_ = 0;
...
public:
    ...
    bool Exit_SleepModem() {
        AtUart* at_uart = GetAtUartFromMl307();
        if (at_uart == nullptr) {
            no_modem = true;
            ESP_LOGE(TAG, "唤醒4G模组失败:未获取到AtUart实例");
            return false;
        }
        if (at_uart->SendCommand("AT+ECPMUCFG=1,1", 50)) {
            ESP_LOGI(TAG, "唤醒4G模组成功");
            return true; 
        } else {
            ESP_LOGE(TAG, "唤醒4G模组失败");
            return false;
        }
    }

    bool Enter_SleepModem() {
        AtUart* at_uart = GetAtUartFromMl307();
        if (at_uart == nullptr) {
            no_modem = true;
            ESP_LOGE(TAG, "唤醒4G模组失败:未获取到AtUart实例");
        ...
        if (at_uart->SendCommand("AT+ECPMUCFG=1,4", 50)) {
            ESP_LOGI(TAG,"休眠4g模组成功");
            return true; 
        } else {
            ESP_LOGE(TAG,"休眠4g模组失败");
            return false;
        }
    }
