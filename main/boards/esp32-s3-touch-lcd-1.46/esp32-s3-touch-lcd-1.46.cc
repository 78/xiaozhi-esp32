...
#include <esp_lcd_spd2010.h>  // 引入SPD2010 LCD驱动库，用于控制SPD2010显示屏
#include <esp_timer.h>  // 引入ESP定时器库，用于定时任务
#include "esp_io_expander_tca9554.h"  // 引入TCA9554 IO扩展器库，用于扩展IO口
#include "lcd_display.h"  // 引入LCD显示库，提供显示功能
#define TAG "waveshare_lcd_1_46"  // 定义日志标签，用于标识日志来源
...
public:
    ...
    static void rounder_event_cb(lv_event_t * e) {
        ...
        area->x1 = (x1 >> 2) << 2;          // 将起始坐标向下舍入到最近的4的倍数
        area->x2 = ((x2 >> 2) << 2) + 3;    // 将结束坐标向上舍入到最近的4的倍数加3
    }
    ...
    CustomLcdDisplay(esp_lcd_panel_io_handle_t io_handle, 
        ...
        lv_display_add_event_cb(display_, rounder_event_cb, LV_EVENT_INVALIDATE_AREA, NULL);  // 添加区域舍入事件回调
    }
};
...
private:
    ...
    void Initializespd2010Display() {
        ...
        const esp_lcd_panel_io_spi_config_t io_config = SPD2010_PANEL_IO_QSPI_CONFIG(QSPI_PIN_NUM_LCD_CS, NULL, NULL);  // 配置面板IO
        ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)QSPI_LCD_HOST, &io_config, &panel_io));  // 创建面板IO

        ESP_LOGI(TAG, "Install SPD2010 panel driver");  // 记录信息日志
        ...
        ESP_ERROR_CHECK(esp_lcd_new_panel_spd2010(panel_io, &panel_config, &panel));  // 创建SPD2010面板
...
public:
    ...
    CustomBoard() :
        ...
        Initializespd2010Display();  // 初始化SPD2010显示屏
    ...
    virtual AudioCodec* GetAudioCodec() override {
        static NoAudioCodecSimplex audio_codec(AUDIO_INPUT_SAMPLE_RATE, AUDIO_OUTPUT_SAMPLE_RATE,
            AUDIO_I2S_SPK_GPIO_BCLK, AUDIO_I2S_SPK_GPIO_LRCK, AUDIO_I2S_SPK_GPIO_DOUT, I2S_STD_SLOT_LEFT, AUDIO_I2S_MIC_GPIO_SCK, AUDIO_I2S_MIC_GPIO_WS, AUDIO_I2S_MIC_GPIO_DIN, I2S_STD_SLOT_RIGHT);  // 创建无音频编解码器对象
