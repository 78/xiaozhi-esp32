...
#define TAG "waveshare_lcd_3_49b"
...
private:
    ...
    void InitializeTca9554(void) {
        ...
        // On this hardware revision the LCD reset and backlight enable lines
        // are wired to the TCA9554 IO expander instead of direct GPIOs.
        ret = esp_io_expander_set_dir(io_expander, IO_EXPANDER_PIN_NUM_1 | IO_EXPANDER_PIN_NUM_5, IO_EXPANDER_OUTPUT);
        ESP_ERROR_CHECK(ret);
        // Keep backlight disabled and LCD out of reset until the panel is initialized.
        ret = esp_io_expander_set_level(io_expander, IO_EXPANDER_PIN_NUM_1, 0);
        ESP_ERROR_CHECK(ret);
        ret = esp_io_expander_set_level(io_expander, IO_EXPANDER_PIN_NUM_5, 1);
        ESP_ERROR_CHECK(ret);
    }
    ...
    // Reset the LCD through the TCA9554 IO expander's LCD_RST pin.
    void ResetLcdViaIoExpander() {
    ...
    void InitializeLcdDisplay() {
        ...
        ResetLcdViaIoExpander();
        esp_lcd_panel_init(panel);

        display_ = new CustomLcdDisplay(panel_io, panel,
                                    DISPLAY_WIDTH, DISPLAY_HEIGHT, DISPLAY_OFFSET_X, DISPLAY_OFFSET_Y, DISPLAY_MIRROR_X, DISPLAY_MIRROR_Y, DISPLAY_SWAP_XY);

        // Enable backlight power now that the panel has been initialized;
        // brightness is subsequently controlled via PWM (see GetBacklight()).
