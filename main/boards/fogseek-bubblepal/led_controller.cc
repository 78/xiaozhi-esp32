...
// 添加RGB灯带句柄
static led_strip_handle_t rgb_strip_handle = nullptr;
...
LedController::~LedController()
{
    ...
    if (rgb_strip_handle)
    {
        led_strip_del(rgb_strip_handle);
        rgb_strip_handle = nullptr;
    }
}
...
void LedController::InitializeLeds(PowerManager &power_manager)
{
    ...
    strip_config.strip_gpio_num = LCD_RGB_IN_GPIO;
    ...
    ESP_ERROR_CHECK(led_strip_new_rmt_device(&strip_config, &rmt_config, &rgb_strip_handle));
    if (rgb_strip_handle)
    {
        led_strip_clear(rgb_strip_handle);
        led_strip_refresh(rgb_strip_handle);
    }
...
void LedController::SetRgbColor(uint8_t r, uint8_t g, uint8_t b)
{
    if (rgb_strip_handle)
    {
        for (int i = 0; i < 8; i++)
        {
            led_strip_set_pixel(rgb_strip_handle, i, r, g, b);
        }
        led_strip_refresh(rgb_strip_handle);
    }
}

void LedController::TurnOnRgb()
{
