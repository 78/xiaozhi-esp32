...
private:
            ...
            .callback = [](void *arg) {
                ...
                if (self->LcdStatus_ == kDevicelcdbacklightOff && Application::GetInstance().GetDeviceState() == kDeviceStateListening 
                    ...
                    if (self->ticks_ % self->kChgCtrlInterval == 0) {
                        ...
                        if (self->power_manager_->low_voltage_ < 2870) {
                            gpio_set_level(SYS_POW_PIN, 0); /* 关机 */
                        }
                    }
                }
            },
    ...
    void InitializePowerSaveTimer() {
        ...
        power_save_timer_->OnEnterSleepMode([this]() {
            ESP_LOGD(TAG, "Enabling sleep mode");
    ...
    void InitializeButtons() {
        ...
        right_button_.OnLongPress([this]() {
            //电池供电时，断电
            if (power_status_ == kDeviceBatterySupply && LcdStatus_ != kDevicelcdbacklightOff) {    
                gpio_set_level(SYS_POW_PIN, 0); 
            } else if (power_status_ == kDeviceTypecSupply && LcdStatus_ == kDevicelcdbacklightOn && Application::GetInstance().GetDeviceState() != kDeviceStateStarting) { //伪关机
                Application::GetInstance().StartListening();
                GetBacklight()->SetBrightness(0); 
                LcdStatus_ = kDevicelcdbacklightOff;
                ESP_LOGI(TAG, "lcd backlight off");
            } else if (LcdStatus_ == kDevicelcdbacklightOff && (power_status_ == kDeviceTypecSupply || power_status_ == kDeviceBatterySupply)) { //typec插电关机后，重新开机
