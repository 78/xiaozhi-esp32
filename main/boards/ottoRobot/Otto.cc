...
// NVS 命名空间
static const char* STORAGE_NAMESPACE = "otto_cfg";
...
void Otto::init(int YL, int YR, int RL, int RR, bool load_calibration) {
    ...
    if (load_calibration) {
        ...
        if (err == ESP_OK) {
            ...
            for (int i = 0; i < 4; i++) {
                char key[10];
                sprintf(key, "trim_%d", i);

                int32_t servo_trim = 0;
                err = nvs_get_i32(my_handle, key, &servo_trim);

                if (err == ESP_OK) {
                    // 设置校准值
                    servo[i].SetTrim(servo_trim);
                    ESP_LOGI(TAG, "Loaded trim value for servo %d: %d", i, (int)servo_trim);
                } else if (err == ESP_ERR_NVS_NOT_FOUND) {
                    ESP_LOGI(TAG, "Trim value for servo %d not found", i);
                } else {
                    ESP_LOGE(TAG, "Error reading trim value for servo %d: %s", i,
                             esp_err_to_name(err));
                }
            }
...
void Otto::saveTrimsOnEEPROM() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        int32_t trim_value = servo[i].getTrim();
        err = nvs_set_i32(my_handle, key, trim_value);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Error setting trim value for servo %d: %s", i, esp_err_to_name(err));
        }
    }
    ...
    // 关闭 NVS 句柄
    nvs_close(my_handle);
    ESP_LOGI(TAG, "Servo trims saved to NVS");
}
