...
#define TAG "ZHENGCHEN_LcdDisplay"
...
static const ThemeColors DARK_THEME = {
    ...
    .high_temp = DARK_LOW_BATTERY_COLOR
};
...
static const ThemeColors LIGHT_THEME = {
    ...
    .high_temp = LIGHT_LOW_BATTERY_COLOR
};
...
SpiZHENGCHEN_LcdDisplay::SpiZHENGCHEN_LcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
                           int width, int height, int offset_x, int offset_y, bool mirror_x, bool mirror_y, bool swap_xy,
                           DisplayFonts fonts)
    : ZHENGCHEN_LcdDisplay(panel_io, panel, fonts) {
    width_ = width;
    height_ = height;
...
// RGB LCD实现
RgbZHENGCHEN_LcdDisplay::RgbZHENGCHEN_LcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
...
ZHENGCHEN_LcdDisplay::~ZHENGCHEN_LcdDisplay() {
    // 然后再清理 LVGL 对象
    if (content_ != nullptr) {
        lv_obj_del(content_);
    }
...
bool ZHENGCHEN_LcdDisplay::Lock(int timeout_ms) {
    return lvgl_port_lock(timeout_ms);
}

void ZHENGCHEN_LcdDisplay::Unlock() {
    lvgl_port_unlock();
}

#if CONFIG_USE_WECHAT_MESSAGE_STYLE
void ZHENGCHEN_LcdDisplay::SetupUI() {
    ...
    lv_obj_set_style_bg_color(high_temp_popup_, current_theme.high_temp, 0);
    lv_obj_set_style_radius(high_temp_popup_, 10, 0);
    lv_obj_t* high_temp_label = lv_label_create(high_temp_popup_);
    lv_label_set_text(high_temp_label, "警告：温度过高");
...
void ZHENGCHEN_LcdDisplay::SetChatMessage(const char* role, const char* content) {
    DisplayLockGuard lock(this);
    if (content_ == nullptr) {
        return;
    }
...
void ZHENGCHEN_LcdDisplay::SetEmotion(const char* emotion) {
    struct Emotion {
        const char* icon;
        const char* text;
    };
...
void ZHENGCHEN_LcdDisplay::SetIcon(const char* icon) {
    DisplayLockGuard lock(this);
    if (emotion_label_ == nullptr) {
        return;
    }
...
void ZHENGCHEN_LcdDisplay::SetTheme(const std::string& theme_name) {
    DisplayLockGuard lock(this);
