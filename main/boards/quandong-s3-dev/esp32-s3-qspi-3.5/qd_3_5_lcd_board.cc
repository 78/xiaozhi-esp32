...
#define TAG "QuandongS3DevBoard_3.5"
...
{0xF1, (uint8_t []){0x00}, 1, 0},
    ...
    {0xF1, (uint8_t []){0x00}, 1, 0},
    {0x71, (uint8_t []){0xD0}, 1, 0},//0xC0
    {0x66, (uint8_t []){0x02, 0x3F}, 2,  0},
    {0xBE, (uint8_t []){0x26, 0x00, 0x9D}, 3, 0},//0x24
    {0x70, (uint8_t []){0x01, 0xA6, 0x11, 0x40, 0xE0, 0x00, 0x11, 0x60, 0x11, 0x00, 0x00, 0x1A}, 12, 0}, //2-0xA0 8-0x69
    {0x90, (uint8_t []){0x04, 0x04, 0x55, 0x74, 0x00, 0x40, 0x43, 0x2D, 0x2D}, 9, 0},//8,9-0x27
    {0x91, (uint8_t []){0x04, 0x04, 0x55, 0x75, 0x00, 0x40, 0x42, 0x2D, 0x2D}, 9, 0},//8,9-0x27
    {0x92, (uint8_t []){0x04, 0x44, 0x55, 0xC0, 0x06, 0x00, 0x07, 0x05, 0x90, 0x2D}, 10, 0},//10-0x27
    {0x93, (uint8_t []){0x04, 0x43, 0x11, 0x00, 0x00, 0x00, 0x00, 0x05, 0x90, 0x2D}, 10, 0},//10-0x27
    {0x94, (uint8_t []){0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, 6, 0},
    {0x95, (uint8_t []){0x96, 0x16, 0x00, 0x00, 0xFF}, 5, 0},
    {0x96, (uint8_t []){0x44, 0x53, 0x03, 0x12, 0x23, 0x24, 0x06, 0x05, 0x9A, 0x2D, 0x00, 0x44}, 12, 0},//9-0x94,10-0x27
    {0x97, (uint8_t []){0x44, 0x53, 0x47, 0x56, 0x20, 0x20, 0x02, 0x01, 0x9A, 0x2D, 0x00, 0x44}, 12, 0},//9-0x94,10-0x27
    {0xBA, (uint8_t []){0x55, 0x9A, 0x2D, 0x9A, 0x2D}, 5, 0}, //2,4-0x94,5-0x27
    ...
    {0x86, (uint8_t []){0xC6, 0x04, 0xB1, 0x02, 0x58, 0x12, 0x58, 0x0C, 0x13, 0x01, 0xA5, 0x00, 0xA5, 0xA5}, 14, 0},//1-0xCD
    ...
    {0x3A, (uint8_t []){0x01}, 1, 0},//0x01
    {0x36, (uint8_t []){0x00}, 1, 0},
    {0x35, (uint8_t []){0x01}, 1, 20},
};

// ---- ST77922 要求刷新区按 4 像素对齐 ----
...
class St77922Display : public LcdDisplay {
private:
...
public:
    St77922Display(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel, int width,
...
class Qd35LcdBoard : public WifiBoard {
private:
    Button boot_button_;
    i2c_master_bus_handle_t i2c_bus_;
    St77922Display* display_ = nullptr;
    ...
    void InitializeLcdDisplay() {
        ...
        ESP_LOGD(TAG, "Install QSPI panel IO");
        ...
        ESP_LOGD(TAG, "Install ST77922 LCD driver");
        ...
        display_ = new St77922Display(panel_io, panel, DISPLAY_WIDTH, DISPLAY_HEIGHT,
...
public:
    Qd35LcdBoard() : boot_button_(BOOT_BUTTON_GPIO) {
        InitializeI2c();
        InitializeQspiBus();
