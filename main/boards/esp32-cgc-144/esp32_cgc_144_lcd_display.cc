...
#define TAG "ESP32_CGC_144_LcdDisplay"
#define LCD_LEDC_CH LEDC_CHANNEL_0

LV_FONT_DECLARE(font_awesome_30_4);

ESP32_CGC_144_LcdDisplay::ESP32_CGC_144_LcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    const esp_timer_create_args_t timer_args = {
        .callback = [](void* arg) {
            ESP32_CGC_144_LcdDisplay* display = static_cast<ESP32_CGC_144_LcdDisplay*>(arg);
...
ESP32_CGC_144_LcdDisplay::~ESP32_CGC_144_LcdDisplay() {
...
void ESP32_CGC_144_LcdDisplay::InitializeBacklight(gpio_num_t backlight_pin) {
...
void ESP32_CGC_144_LcdDisplay::OnBacklightTimer() {
    ...
    // 修改亮度计算逻辑，使用100 - current_brightness_
    uint32_t duty_cycle = (1023 * (100 - current_brightness_)) / 100;
...
void ESP32_CGC_144_LcdDisplay::SetBacklight(uint8_t brightness) {
...
bool ESP32_CGC_144_LcdDisplay::Lock(int timeout_ms) {
    return lvgl_port_lock(timeout_ms);
}

void ESP32_CGC_144_LcdDisplay::Unlock() {
    lvgl_port_unlock();
}

void ESP32_CGC_144_LcdDisplay::SetupUI() {
...
void ESP32_CGC_144_LcdDisplay::SetEmotion(const char* emotion) {
    struct Emotion {
        const char* icon;
        const char* text;
    };
...
void ESP32_CGC_144_LcdDisplay::SetIcon(const char* icon) {
    DisplayLockGuard lock(this);
    if (emotion_label_ == nullptr) {
        return;
    }
