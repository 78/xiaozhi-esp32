...
#define TAG "waveshare_3.49_lcd_display"
...
const userThemeColors DARK_THEME = {
...
const userThemeColors LIGHT_THEME = {
...
userLcdDisplay::userLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel, DisplayFonts fonts, int width, int height)
...
userSpiLcdDisplay::userSpiLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
                           int width, int height, int offset_x, int offset_y, bool mirror_x, bool mirror_y, bool swap_xy,
                           DisplayFonts fonts)
    : userLcdDisplay(panel_io, panel, fonts, width, height) {
...
bool userLcdDisplay::Lock(int timeout_ms) {
    return lvgl_port_lock(timeout_ms);
}

void userLcdDisplay::Unlock() {
    lvgl_port_unlock();
}
...
void userLcdDisplay::SetEmotion(const char* emotion) {
    struct Emotion {
        const char* icon;
        const char* text;
    };
...
void userLcdDisplay::SetIcon(const char* icon) {
    DisplayLockGuard lock(this);
    if (emotion_label_ == nullptr) {
        return;
    }
...
void userLcdDisplay::SetPreviewImage(const lv_img_dsc_t* img_dsc) {
    DisplayLockGuard lock(this);
    if (preview_image_ == nullptr) {
        return;
    }
...
void userLcdDisplay::SetTheme(const std::string& theme_name) {
    DisplayLockGuard lock(this);
