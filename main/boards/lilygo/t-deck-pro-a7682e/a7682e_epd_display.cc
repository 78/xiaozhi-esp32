...
constexpr const char* kTag = "A7682eEpdDisplay";
...
A7682eEpdDisplay::A7682eEpdDisplay()
    : LcdDisplay(nullptr, nullptr, T_DECK_PRO_A7682E_EPD_WIDTH, T_DECK_PRO_A7682E_EPD_HEIGHT) {
    InitializeGpio();
    if (!InitializeSpi()) {
        return;
    }
    ...
    display_ = lv_display_create(T_DECK_PRO_A7682E_EPD_WIDTH, T_DECK_PRO_A7682E_EPD_HEIGHT);
    if (display_ != nullptr) {
...
void A7682eEpdDisplay::InitializeGpio() {
    gpio_config_t output_config = {};
    output_config.pin_bit_mask =
        (1ULL << T_DECK_PRO_A7682E_EPD_CS) | (1ULL << T_DECK_PRO_A7682E_EPD_DC) |
        (1ULL << T_DECK_PRO_A7682E_EPD_RST) | (1ULL << T_DECK_PRO_A7682E_EPD_BL) |
        (1ULL << T_DECK_PRO_A7682E_LORA_CS) | (1ULL << T_DECK_PRO_A7682E_LORA_RST) |
        (1ULL << T_DECK_PRO_A7682E_SD_CS);
    ...
    busy_config.pin_bit_mask = 1ULL << T_DECK_PRO_A7682E_EPD_BUSY;
    ...
    // The EPD, LoRa and SD card share the SPI signals. Keep every device deselected.
    gpio_set_level(T_DECK_PRO_A7682E_LORA_CS, 1);
    gpio_set_level(T_DECK_PRO_A7682E_LORA_RST, 1);
    gpio_set_level(T_DECK_PRO_A7682E_SD_CS, 1);
    gpio_set_level(T_DECK_PRO_A7682E_EPD_CS, 1);
    gpio_set_level(T_DECK_PRO_A7682E_EPD_DC, 1);
    gpio_set_level(T_DECK_PRO_A7682E_EPD_RST, 1);
    gpio_set_level(T_DECK_PRO_A7682E_EPD_BL, 1);
}

bool A7682eEpdDisplay::InitializeSpi() {
    spi_bus_config_t bus_config = {};
    bus_config.mosi_io_num = T_DECK_PRO_A7682E_EPD_MOSI;
    bus_config.miso_io_num = -1;
    bus_config.sclk_io_num = T_DECK_PRO_A7682E_EPD_SCK;
    bus_config.quadwp_io_num = -1;
    bus_config.quadhd_io_num = -1;
    bus_config.max_transfer_sz = kSpiMaxTransferBytes;

    esp_err_t ret =
        spi_bus_initialize(T_DECK_PRO_A7682E_EPD_SPI_HOST, &bus_config, SPI_DMA_CH_AUTO);
    if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
        ESP_LOGE(kTag, "EPD SPI bus setup failed: %s", esp_err_to_name(ret));
        return false;
    }
    spi_bus_owned_ = ret == ESP_OK;
    ...
    ret = spi_bus_add_device(T_DECK_PRO_A7682E_EPD_SPI_HOST, &device_config, &spi_);
    if (ret != ESP_OK) {
...
void A7682eEpdDisplay::HardwareReset() {
    gpio_set_level(T_DECK_PRO_A7682E_EPD_RST, 1);
    vTaskDelay(pdMS_TO_TICKS(10));
    gpio_set_level(T_DECK_PRO_A7682E_EPD_RST, 0);
...
bool A7682eEpdDisplay::WaitWhileBusy(const char* operation) {
    const int64_t deadline = esp_timer_get_time() + kBusyTimeoutUs;
    vTaskDelay(pdMS_TO_TICKS(1));
    while (gpio_get_level(T_DECK_PRO_A7682E_EPD_BUSY) == kEpdBusyLevel) {
...
void A7682eEpdDisplay::SendCommand(uint8_t command) {
    if (spi_ == nullptr) {
        return;
    }
    ...
    gpio_set_level(T_DECK_PRO_A7682E_EPD_DC, 0);
    gpio_set_level(T_DECK_PRO_A7682E_EPD_CS, 0);
...
void A7682eEpdDisplay::SendData(uint8_t data) {
    if (spi_ == nullptr) {
        return;
    }
    spi_transaction_t transaction = {};
...
bool A7682eEpdDisplay::SendBytes(const uint8_t* data, size_t size) {
...
bool A7682eEpdDisplay::NormalizeWindow(const lv_area_t* area, EpdWindow* window) const {
    ...
    const int x2 = std::min(T_DECK_PRO_A7682E_EPD_WIDTH - 1, static_cast<int>(area->x2));
    const int y2 = std::min(T_DECK_PRO_A7682E_EPD_HEIGHT - 1, static_cast<int>(area->y2));
    if (x1 > x2 || y1 > y2) {
        return false;
    }
    ...
    const uint16_t aligned_x_end =
        static_cast<uint16_t>(std::min(T_DECK_PRO_A7682E_EPD_WIDTH - 1, x2 | 0x0007));
    window->x = aligned_x;
...
void A7682eEpdDisplay::SetPartialRamArea(const EpdWindow& window) {
...
void A7682eEpdDisplay::InitializePanelRegisters() {
...
bool A7682eEpdDisplay::WriteFullFrameBuffer(uint8_t command) {
...
bool A7682eEpdDisplay::WriteFrameRegion(uint8_t command, const EpdWindow& window) {
    ...
    for (uint16_t row = 0; row < window.height; ++row) {
        const uint8_t* row_data =
            frame_buffer_ +
            (static_cast<size_t>(window.y + row) * (T_DECK_PRO_A7682E_EPD_WIDTH / 8)) +
            (window.x / 8);
        spi_transaction_t transaction = {};
...
bool A7682eEpdDisplay::PowerOff() {
    if (!power_is_on_) {
        return true;
    }
    SendCommand(0x02);
...
bool A7682eEpdDisplay::Refresh(const EpdWindow& window, bool full_refresh) {
    if (!full_refresh) {
        SendCommand(0x91);  // partial RAM area in
        SetPartialRamArea(window);
    }
...
bool A7682eEpdDisplay::InitializePanel() {
    HardwareReset();
    memset(frame_buffer_, 0xFF, kFrameBytes);
...
void A7682eEpdDisplay::UpdateFrameBuffer(const lv_area_t* area, const uint8_t* color_buffer) {
    ...
    const int x_end = std::min(T_DECK_PRO_A7682E_EPD_WIDTH - 1, static_cast<int>(area->x2));
    const int y_end = std::min(T_DECK_PRO_A7682E_EPD_HEIGHT - 1, static_cast<int>(area->y2));
    if (x_start > x_end || y_start > y_end) {
        return;
    }

    const size_t stride = T_DECK_PRO_A7682E_EPD_WIDTH / 8;
...
void A7682eEpdDisplay::LvglFlushCallback(lv_display_t* display, const lv_area_t* area,
                                         uint8_t* color_buffer) {
    auto* driver = static_cast<A7682eEpdDisplay*>(lv_display_get_user_data(display));
    if (driver == nullptr) {
...

    {
        ...
        A7682eEpdDisplay::EpdWindow window{};
        if (driver->panel_ready_ && driver->NormalizeWindow(area, &window)) {
            ...
            ESP_LOGD(kTag, "LVGL flush area=(%d,%d)-(%d,%d), window=(%u,%u)+(%u,%u), mode=%s",
                     area->x1, area->y1, area->x2, area->y2, window.x, window.y, window.width,
                     window.height, driver->first_content_refresh_ ? "full-pending" : "partial");
        }
    }
    lv_display_flush_ready(display);
}
