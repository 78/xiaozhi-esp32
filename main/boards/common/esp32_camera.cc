...
Esp32Camera::Esp32Camera(const esp_video_init_config_t& config) {
    ...
    if (video_fd_ < 0) {
        ESP_LOGE(TAG, "open %s failed", video_device_name);
        return;
    }
    ...
    ESP_LOGI(TAG, "VIDIOC_G_FMT: pixelformat=0x%08x, width=%d, height=%d", format.fmt.pix.pixelformat,
    ...
    while (ioctl(video_fd_, VIDIOC_ENUM_FMT, &fmtdesc) == 0) {
        ESP_LOGI(TAG, "VIDIOC_ENUM_FMT: pixelformat=0x%08x, description=%s", fmtdesc.pixelformat, fmtdesc.description);
    ...
    ESP_LOGI(TAG, "selected pixel format: 0x%08x", setformat.fmt.pix.pixelformat);
