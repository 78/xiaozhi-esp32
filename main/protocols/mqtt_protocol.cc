...
bool MqttProtocol::StartMqttClient() {
    ...
    if (!mqtt_->Connect(endpoint_, 8883, client_id_, username_, password_)) {
        ...
        if (on_network_error_ != nullptr) {
            on_network_error_(LANG_TXT_UNABLE_TO_CONNECT_TO_SERVICE);
        }
        return false;
    }
...
void MqttProtocol::SendText(const std::string& text) {
    ...
    if (!mqtt_->Publish(publish_topic_, text)) {
        ...
        if (on_network_error_ != nullptr) {
            on_network_error_(LANG_TXT_SENDING_FAILED_PLEASE_CHECK_THE_NETWORK);
        }
    }
}
...
bool MqttProtocol::OpenAudioChannel() {
    ...
    if (!(bits & MQTT_PROTOCOL_SERVER_HELLO_EVENT)) {
        ...
        if (on_network_error_ != nullptr) {
            on_network_error_(LANG_TXT_WAITING_FOR_RESPONSE_TIMEOUT);
        }
        return false;
    }
