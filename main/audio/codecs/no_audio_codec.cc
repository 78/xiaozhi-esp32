...
int NoAudioCodec::Write(const int16_t* data, int samples) {
    ...
    // Limit max volume to 80 for battery safety to prevent brownout reset
    int effective_volume = std::min(output_volume_, 80);
    int32_t volume_factor = pow(double(effective_volume) / 100.0, 2) * 65536;
