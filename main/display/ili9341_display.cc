...
static void ili9341_lvgl_port_update_callback(lv_disp_drv_t *drv)
{
    esp_lcd_panel_handle_t panel_handle = (esp_lcd_panel_handle_t)drv->user_data;
...
#if CONFIG_ILI9341_LCD_TOUCH_ENABLED
        // Rotate LCD touch
...
Ili9341Display::Ili9341Display(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    ESP_ERROR_CHECK(esp_timer_start_periodic(lvgl_tick_timer_, ILI9341_LVGL_TICK_PERIOD_MS * 1000));

    lvgl_mutex_ = xSemaphoreCreateRecursiveMutex();
    assert(lvgl_mutex_ != nullptr);
    ESP_LOGI(TAG, "Create LVGL task");
    xTaskCreate([](void *arg) {
        static_cast<Ili9341Display*>(arg)->LvglTask();
        vTaskDelete(NULL);
    }, "LVGL", ILI9341_LVGL_TASK_STACK_SIZE, this, ILI9341_LVGL_TASK_PRIORITY, NULL);
...
Ili9341Display::~Ili9341Display() {
    ...
    if (network_label_ != nullptr) {
        lv_obj_del(ai_messge_label_);
        lv_obj_del(user_messge_label_);
    }
...
void Ili9341Display::InitializeBacklight(gpio_num_t backlight_pin) {
...
void Ili9341Display::SetBacklight(uint8_t brightness) {
...
bool Ili9341Display::Lock(int timeout_ms) {
...
void Ili9341Display::Unlock() {
    xSemaphoreGiveRecursive(lvgl_mutex_);
}

void Ili9341Display::SetChatMessage(const std::string &role, const std::string &content) {
