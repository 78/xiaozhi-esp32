...
#define DEFAULT_TOOLCALL_STACK_SIZE 12288
...
void McpServer::AddCommonTools() {
...
#else
  // For non-LVGL displays, use simpler theme support
  auto display = board.GetDisplay();
  if (display && !display->GetTheme().empty()) {
    AddTool("self.screen.set_theme",
            ...
            [display](const PropertyList &properties) -> ReturnValue {
              display->SetTheme(
                  properties["theme"].value<std::string>().c_str());
              return true;
            });
  }
  ...
  if (camera) {
    AddTool("self.camera.take_photo",
            ...
            [camera](const PropertyList &properties) -> ReturnValue {
              if (!camera->Capture()) {
                return "{\"success\": false, \"message\": \"Failed to capture "
                       "photo\"}";
              }
              auto question = properties["question"].value<std::string>();
              return camera->Explain(question);
...
void McpServer::ParseMessage(const std::string &message) {
  ESP_LOGI(TAG, "Received MCP message: %s", message.c_str());
...
void McpServer::DoToolCall(int id, const std::string &tool_name,
  ...
  tool_call_thread_ =
      std::thread([this, id, tool_iter, arguments = std::move(arguments)]() {
        try {
